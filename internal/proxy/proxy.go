...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if s.postureClient != nil {
		...
		if err != nil {
			...
			postureFacts = policy.PostureFacts{DiskEncrypted: report.DiskEncrypted, EDRRunning: report.EDRRunning, PatchLevel: report.PatchLevel, ReportedAt: report.CheckedAt}
		}
	}
	...
	if s.postureClient != nil && !postureOK {
		s.deny(w, r, auth.subject, auth.spiffeID, auth.authMethod, start, "policy matched but device posture is missing or stale (failing closed)", decision.PolicyID, riskResult)
		return
	}
...
func (s *Server) recentDenials(subject string) int {
	count := 0
	for _, e := range s.accessLog.Recent(20) {
		if e.Subject == subject && !e.Allowed {
			count++
		}
	}
	return count
