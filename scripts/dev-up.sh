...
# missing, and starts the posture agent + IAP proxy against the bundled
# example policy set. No Docker, no Vault, no SPIRE server required — the
# SPIFFE-shaped certs come from certs/generate-certs.sh instead.
#
# For the full SPIRE/Vault/Envoy reference deployment, see deploy/README.md.
set -euo pipefail
cd "$(dirname "$0")/.."
...
trap 'echo ""; echo "==> Shutting down"; kill "$POSTURE_PID" "$PROXY_PID" 2>/dev/null || true' EXIT INT TERM

sleep 1
echo ""
echo "Ready. Try:"
echo "  curl -k --cacert certs/out/ca.crt --cert certs/out/alice.crt --key certs/out/alice.key https://localhost:8443/"
